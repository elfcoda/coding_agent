...
def _is_debugger_attached() -> bool:
    """Detect whether the current process is running under a Python debugger."""
    trace_func = getattr(sys, "gettrace", None)
    return bool(trace_func and trace_func())
...
def start_web_app(host: str = "127.0.0.1", port: int = 5777, reload: bool = False):
    ...
    if reload and _is_debugger_attached():
        logger.warning("Debugger detected; disabling --reload to avoid debugpy subprocess crashes on Windows")
    ...
    uvicorn.run("web_app:app", host=host, port=port, reload=effective_reload, **reload_kwargs)
