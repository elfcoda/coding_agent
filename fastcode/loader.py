...
class RepositoryLoader:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.clone_retries = max(1, int(self.repo_config.get("clone_retries", 3)))
        self.clone_retry_delay_seconds = max(
            0,
            int(self.repo_config.get("clone_retry_delay_seconds", 2))
        )
    ...
    def _remove_readonly(func, path, exc_info):
        """Allow cleanup of read-only Git files on Windows."""
    ...
    def load_from_url(self, url: str, target_dir: Optional[str] = None) -> str:
        ...
        base_dir = os.path.abspath(target_dir) if target_dir else self.safe_repo_root
        ensure_dir(base_dir)
        self.repo_path = os.path.join(base_dir, self.repo_name)
        backup_path = None

        if os.path.exists(self.repo_path):
            backup_path = self._backup_existing_repo(self.repo_path)

        last_error = None

        for attempt in range(1, self.clone_retries + 1):
            try:
                ...
                last_error = e
                self.logger.warning(
                    f"Clone attempt {attempt}/{self.clone_retries} failed for {url}: {e}"
                )

                if os.path.exists(self.repo_path):
                    shutil.rmtree(self.repo_path, onerror=self._remove_readonly)

                if attempt < self.clone_retries and self.clone_retry_delay_seconds > 0:
                    time.sleep(self.clone_retry_delay_seconds)

        if backup_path and not os.path.exists(self.repo_path):
            shutil.move(backup_path, self.repo_path)
            self.logger.info(f"Restored previous repository backup to {self.repo_path}")

        self.logger.error(f"Failed to clone repository after {self.clone_retries} attempts: {last_error}")
        raise RuntimeError(f"Failed to clone repository after {self.clone_retries} attempts: {last_error}")
