...
decision-first和低熵体项目：
一个不断输入不断决策（两类输入）（各种类别的decision）的系统：（强迫开发者去掌握本该自己掌握的部分，阻止失控）decision剥离分发器，你不断给decision（agent决定的高并行低延迟的依据置信度，用户要思考，要decision然后，agent把代码写好）拆分项目，可以持续输入给core不同信息，不用新开session，他们会自动整合到一起，但是互相隔离。全部展示。能想到的最高效的能掌控代码还能提效的agent模式。agent的垂直工作流整合。like a game。所以需要这样的ui体感。
...
状态转移：(复杂，但属于verify和重构才需要的功能，核心demo不需要，核心demo只有高并行只有decision-first概念)verify没那么重要，重要的是你掌握了系统。
1. 依赖的DependencyEdgeRecord都已经inactive
2. impl_on_contracts里是的contract都已经完结
3. 当前work item的执行进程已结束
当以上3个条件都满足，当前work item就取消block阻塞状态。
...
1. 当一个work item进程结束返回给core manager后
2. 当scheduler循环扫描发现有contract的状态发生变化为完结
3. 当某个work item被取消block状态导致依赖边变成inactive后（这种情况会遍历dfs graph不断传播完成状态）
    ...
    最后：要能做demo，但是系统架构，状态流转，设计原理，核心流程要懂。其他的不用做，不用做完整，没人问，但能说清楚。所有e2e测试跑通就差不多核心完成了。
